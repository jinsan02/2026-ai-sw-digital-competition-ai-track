...
def tune_class_bias(logits, y_true, rounds=3):
    """개선 1: per-class additive logit bias를 Macro-F1 직접 최대화로 coordinate-ascent.

    coarse(±3.0 step 0.1) → fine(±3.0 step 0.02) 2단계 그리드로 세분화해 하위 클래스
    결정경계를 더 촘촘히 최적화. 검증 로짓에서만 fit (호출부에서 held-out val logits 전달).
    """
    coarse = [x / 10.0 for x in range(-30, 31)]        # ±3.0, step 0.1 (61)
    fine = [x / 50.0 for x in range(-150, 151)]        # ±3.0, step 0.02 (301)
    bias = torch.zeros(len(ALL_CLASSES), dtype=torch.float32)
    best = f1_metrics(y_true, predict_with_bias(logits, bias))["macro_f1"]

    for stage, candidates in enumerate((coarse, fine), 1):
        for round_no in range(rounds):
            ...
            for class_id, class_name in enumerate(ALL_CLASSES):
                ...
                for candidate in candidates:
                    ...
                    score = f1_metrics(y_true, predict_with_bias(logits, trial))["macro_f1"]
                ...
                if local_best > best + 1e-7:
                    ...
                    best = local_best
                    improved = True
                    print(f"  bias stage={stage} round={round_no + 1} class={class_name} "
                          f"value={local_value:.3f} macro_f1={best:.5f}")
                else:
